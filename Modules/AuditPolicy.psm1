...
function New-AuditPolicyResult {param([string]$ControlID,[string]$Title,[string]$Expected,[string]$Actual,[ValidateSet('PASS','FAIL','WARNING','NOT APPLICABLE')][string]$Status,[ValidateSet('Critical','High','Medium','Low','Informational')][string]$Severity,[string]$Evidence,[string]$Remediation)New-CISResult -ControlID $ControlID -Category 'Audit Policy' -SubCategory 'Advanced Audit Policy' -Title $Title -Expected $Expected -Actual $Actual -Status $Status -Severity $Severity -Evidence $Evidence -Remediation $Remediation -Reference 'CIS Microsoft Windows 10/11 Benchmark - Advanced Audit Policy Configuration'}
function Invoke-Audit {
    [CmdletBinding()][OutputType([System.Management.Automation.PSCustomObject])]param()
    $auditpol=Join-Path $env:SystemRoot 'System32\auditpol.exe';$categories=@('Account Logon','Logon','Account Management','Policy Change','Privilege Use','Object Access','DS Access','Detailed Tracking','System');$id=0
